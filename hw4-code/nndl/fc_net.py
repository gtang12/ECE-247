...
class FullyConnectedNet(object):
  ...
  def loss(self, X, y=None):
    ...
    for i in np.arange(n_hidden)+1:
      ...
      if self.use_batchnorm:
        a, cache['bn'+str(i)] = batchnorm_forward(a, self.params['gamma'+str(i)], self.params['beta'+str(i)], self.bn_params[i-1])
      b, cache['relu'+str(i)] = relu_forward(a)
      if self.use_dropout:
        b, cache['drop'+str(i)] = dropout_forward(b, self.dropout_param)
    ...
    # hidden layers: [dropout] -> relu -> [batchnorm] -> affine
    for i in np.arange(self.num_layers-1, 0, -1):
      if self.use_dropout:
        db = dropout_backward(db, cache['drop'+str(i)])

      da = relu_backward(db, cache['relu'+str(i)])

      if self.use_batchnorm:
        da, grads['gamma'+str(i)], grads['beta'+str(i)] = batchnorm_backward(da, cache['bn'+str(i)])
