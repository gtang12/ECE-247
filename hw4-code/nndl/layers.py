...
def affine_backward(dout, cache):
  ...
  dx = np.reshape(dout.dot(w.T), (x.shape[0], *x.shape[1:])) # (N,d1,...,dk)
...
def batchnorm_backward(dout, cache):
  ...
  dx_norm = dout*cache['gamma'] # (N,D)
  xc = cache['x'] - cache['mean'] # centered x, # (N,D)
  var_eps = cache['var'] + cache['eps']

  dvar = np.sum(-0.5/var_eps**1.5 * xc * dx_norm, axis=0) # (D,)
  dmean = -1/np.sqrt(var_eps)*np.sum(dx_norm, axis=0) - dvar*2/m*np.sum(xc, axis=0) # (D,)
  dx = 1/np.sqrt(var_eps)*dx_norm + 2*(xc)/m*dvar + 1/m*dmean # (N,D)
