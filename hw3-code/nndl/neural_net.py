...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    H = f(np.dot(W1, X.T) + b1[:,np.newaxis]) # hidden layer, (H,N)
    scores = (np.dot(W2, H) + b2[:,np.newaxis]).T # output layer, (N,C)
    ...
    dLdz = 1/N * (softmax - (y==np.expand_dims(np.arange(W2.shape[0]), axis=1)).T.astype(int)) # (N,C)
    dLdb2 = dLdz.sum(axis=0) # (C,)
    dLdW2 = dLdz.T.dot(H.T) + reg*W2 # (C,H)
    dLdb1 = (W2.T.dot(dLdz.T) * ((W1.dot(X.T))>0).astype(int)).sum(axis=1) # (H,)
    dLdW1 = (W2.T.dot(dLdz.T) * ((W1.dot(X.T))>0).astype(int)).dot(X) + reg*W1 # (H,D)
